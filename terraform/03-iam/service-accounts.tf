...
locals {
  ...
  platform_services = {
    "auth-bff" = {
      secrets  = ["auth-bff-cookie-encryption-key", "auth-bff-csrf-secret", "openfga-preshared-key", "platform-client-secret", "mp-admin-client-secret", "mp-storefront-client-secret", "shared-internal-service-key"]
      has_db   = false
